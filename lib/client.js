...
function GitWorkbenchAction(props) {
  ...
  const pendingProposalId = React4.useRef(null);
  const stateRequestRef = React4.useRef({ controller: null, sequence: 0 });
  React4.useEffect(() => {
    const refresh = () => {
  ...
  return React4.createElement(
    ...
    React4.createElement("span", null, "Git"),
    pending ? React4.createElement("span", { className: "gg-workbench-action-dot", "aria-hidden": true }) : null
  );
}
...
function renderFailureDetails(failure2, recoveryReason = "") {
  ...
  return React4.createElement(
    ...
    React4.createElement(
      ...
      React4.createElement("strong", { className: "gg-failure-label" }, "\u539F\u547D\u4EE4"),
      React4.createElement("code", { className: "gg-stepcode gg-failure-value" }, String(failure2.command || ""))
    ),
    React4.createElement(
      "div",
      { className: "gg-failure-row" },
      React4.createElement("strong", { className: "gg-failure-label" }, "\u9519\u8BEF"),
      React4.createElement("pre", { className: "gg-failure-value" }, failureError(failure2))
    ),
    recoveryReason ? React4.createElement(
      "div",
      { className: "gg-failure-row" },
      React4.createElement("strong", { className: "gg-failure-label" }, "\u4FEE\u6B63\u539F\u56E0"),
      React4.createElement("div", { className: "gg-failure-value" }, recoveryReason)
    ) : null,
    React4.createElement(
      "details",
      null,
      React4.createElement("summary", { className: "gg-failure-label" }, "\u5B8C\u6574\u9519\u8BEF\u4E0E\u4ED3\u5E93\u8BCA\u65AD"),
      React4.createElement("pre", { className: "gg-pre" }, details)
    )
  );
}
...
function useManualRefreshFeedback() {
  ...
  const resetTimerRef = React4.useRef(null);
  ...
  React4.useEffect(() => clearResetTimer, []);
...
function renderDiff(diff) {
  return diff.split("\n").map((line, index) => React4.createElement("span", { className: diffLineClass(line), key: "diff-" + index }, line || " "));
}
function renderReview(diff) {
  const rows = parseReviewRows(diff);
  if (!rows.length) return React4.createElement("div", { className: "gg-idletext" }, diff ? "\u6CA1\u6709\u53EF\u5BA1\u9605\u7684\u4EE3\u7801\u884C\u3002" : "\u6CA1\u6709\u53EF\u663E\u793A\u7684\u5DEE\u5F02\u3002");
  return rows.map((row, index) => {
    if (row.kind === "skipped") return React4.createElement("div", { className: "gg-review-skip", key: "review-" + index }, "\u2304 " + row.text);
    if (row.kind === "annotation") return React4.createElement("div", { className: "gg-review-annotation", key: "review-" + index }, row.text);
    return React4.createElement(
      "div",
      { className: "gg-review-line " + row.kind, key: "review-" + index },
      React4.createElement("span", { className: "gg-review-number" }, row.oldNumber === null ? "" : String(row.oldNumber)),
      React4.createElement("span", { className: "gg-review-number" }, row.newNumber === null ? "" : String(row.newNumber)),
      React4.createElement("code", { className: "gg-review-code" }, row.text || " ")
    );
  });
}
function renderRawDiffSurface(diff) {
  return React4.createElement("code", { className: "gg-diff-content" }, renderDiff(diff));
}
function renderReviewSurface(diff) {
  return React4.createElement("div", { className: "gg-review-content" }, renderReview(diff));
}
function renderRepositoryIdentity(topLevel, branch) {
  ...
  return React4.createElement(
    ...
    React4.createElement("span", { className: "gg-repository-name", title: name }, name),
    React4.createElement("span", { className: "gg-repository-arrow", "aria-hidden": "true" }, "\u2192"),
    React4.createElement("code", { className: "gg-repository-branch", title: currentBranch }, currentBranch)
  );
}
function renderLoadingState(label2) {
  return React4.createElement(
    ...
    React4.createElement(
      ...
      React4.createElement("span", { className: "gg-loading-label" }, label2),
      React4.createElement("span", { className: "gg-loading-bar", "aria-hidden": "true" }),
      React4.createElement("span", { className: "gg-loading-bar", "aria-hidden": "true" }),
      React4.createElement("span", { className: "gg-loading-bar", "aria-hidden": "true" })
    )
  );
...
function GitChangesTab(props) {
  ...
  const [summary, setSummary] = React4.useState(null);
  const [selected, setSelected] = React4.useState(null);
  const [diff, setDiff] = React4.useState("");
  const [busy, setBusy] = React4.useState(false);
  const [message, setMessage] = React4.useState("");
  const [diagnostics, setDiagnostics] = React4.useState("");
  const [commitMessage, setCommitMessage] = React4.useState("");
  const [collapsedFolders, setCollapsedFolders] = React4.useState({});
  const [reviewMode, setReviewMode] = React4.useState("review");
  const refreshFeedback = useManualRefreshFeedback();
  const selectedRef = React4.useRef(null);
  const manualRefreshRef = React4.useRef(false);
  const summaryRequestRef = React4.useRef({ controller: null, sequence: 0 });
  const diffRequestRef = React4.useRef({ controller: null, sequence: 0 });
  ...
  const fileRow = (file, staged, key, depth) => {
    ...
    return React4.createElement(
      ...
      React4.createElement(
        ...
        React4.createElement("code", null, status + " " + String(file.path || ""))
      ),
  ...
  const renderFileTree = (groupFiles, staged, group) => {
    ...
    const renderNode = (node, depth) => {
      ...
      for (const folder of node.folders) {
        ...
        entries.push(React4.createElement(
          "div",
          ...
          React4.createElement(
            ...
            React4.createElement("span", { className: "gg-folder-arrow", "aria-hidden": true }, collapsed ? "\u203A" : "\u2304"),
            React4.createElement("span", { className: "gg-folder-name" }, folder.name + " (" + countFiles(folder) + ")")
          ),
    ...
    return React4.createElement("div", { className: "gg-file-tree" }, renderNode(buildFileTree(groupFiles), 0));
  };
  return React4.createElement(
    "section",
    { className: "gg-tab-content" },
    React4.createElement(
      "div",
      { className: "gg-tab-toolbar" },
      summary ? renderRepositoryIdentity(summary.topLevel, summary.branch || summary.head) : React4.createElement("span", { className: "gg-section-heading" }, "\u6B63\u5728\u8BFB\u53D6\u4ED3\u5E93\u2026"),
      React4.createElement("button", {
        className: "gg-btn",
      ...
      React4.createElement("button", { className: "gg-btn", type: "button", onClick: props.onConflicts }, "\u89E3\u51B3\u51B2\u7A81 / Merge / Cherry-pick"),
      React4.createElement("button", { className: "gg-btn", disabled: busy || !summary || hasConflicts, title: hasConflicts ? "\u8BF7\u5148\u89E3\u51B3\u5E76\u6807\u8BB0\u51B2\u7A81\u6587\u4EF6" : void 0, onClick: () => runMutation("stage-all") }, "\u5168\u90E8\u6682\u5B58"),
      React4.createElement("button", { className: "gg-btn", disabled: busy || !summary, onClick: () => runMutation("unstage-all") }, "\u5168\u90E8\u53D6\u6D88\u6682\u5B58")
    ),
    message ? React4.createElement("div", { className: "gg-workbench-error" }, message) : null,
    diagnostics ? React4.createElement("pre", { className: "gg-diagnostics" }, diagnostics) : null,
    summary ? React4.createElement(
      "div",
      ...
      React4.createElement(
        ...
        React4.createElement(
          ...
          React4.createElement("strong", null, "\u672A\u6682\u5B58", React4.createElement("span", { className: "gg-section-count" }, String(unstagedFiles.length))),
          unstagedFiles.length ? renderFileTree(unstagedFiles, false, "unstaged") : React4.createElement("div", { className: "gg-idletext" }, "\u6CA1\u6709\u672A\u6682\u5B58\u7684\u53D8\u66F4\u3002")
        ),
        React4.createElement(
          "div",
          { className: "gg-file-group" },
          React4.createElement("strong", null, "\u5DF2\u6682\u5B58", React4.createElement("span", { className: "gg-section-count" }, String(stagedFiles.length))),
          stagedFiles.length ? renderFileTree(stagedFiles, true, "staged") : React4.createElement("div", { className: "gg-idletext" }, "\u6CA1\u6709\u5DF2\u6682\u5B58\u7684\u53D8\u66F4\u3002")
        ),
        React4.createElement(
          "div",
          { className: "gg-commit-form" },
          React4.createElement(
            ...
            React4.createElement("span", { className: "gg-field-label" }, "\u63D0\u4EA4\u8BF4\u660E"),
      ...
      React4.createElement(
        "div",
        { className: "gg-diff" },
        React4.createElement("div", { className: "gg-intent" }, selected ? String(selected.path) + (selected.staged ? "\uFF08\u5DF2\u6682\u5B58\uFF09" : "\uFF08\u672A\u6682\u5B58\uFF09") : "\u9009\u62E9\u6587\u4EF6\u4EE5\u67E5\u770B\u5DEE\u5F02"),
        selected ? React4.createElement(
          "div",
          { className: "gg-review-toolbar" },
          React4.createElement("button", { className: "gg-btn gg-review-mode" + (reviewMode === "review" ? " active" : ""), type: "button", onClick: () => setReviewMode("review") }, "\u6587\u4EF6\u5BA1\u9605"),
          React4.createElement("button", { className: "gg-btn gg-review-mode" + (reviewMode === "raw" ? " active" : ""), type: "button", onClick: () => setReviewMode("raw") }, "\u539F\u59CB Diff")
        ) : null,
        selected && reviewMode === "review" ? React4.createElement("div", { className: "gg-review" }, renderReviewSurface(diff)) : React4.createElement(
          "pre",
...
function GitBranchesTab(props) {
  ...
  const [branches, setBranches] = React4.useState([]);
  const [remotes, setRemotes] = React4.useState([]);
  const [tags, setTags] = React4.useState([]);
  const [referenceTab, setReferenceTab] = React4.useState("local");
  const [branchQuery, setBranchQuery] = React4.useState("");
  const [name, setName] = React4.useState("");
  const [base, setBase] = React4.useState("");
  const [busy, setBusy] = React4.useState(false);
  const [message, setMessage] = React4.useState("");
  const [diagnostics, setDiagnostics] = React4.useState("");
  const [confirmDelete, setConfirmDelete] = React4.useState(null);
  const [forceDelete, setForceDelete] = React4.useState(null);
  const [riskAccepted, setRiskAccepted] = React4.useState(false);
  const refreshFeedback = useManualRefreshFeedback();
  const listRequestRef = React4.useRef({ controller: null, sequence: 0 });
  const load = (manual = false) => {
  ...
  const referenceRows = (entries, emptyText) => entries.length ? entries.map((entry, index) => React4.createElement(
    "div",
    { className: "gg-branch-row gg-reference-row", key: String(entry.name || index) },
    React4.createElement("code", null, String(entry.name || "")),
    React4.createElement("code", { className: "gg-reference-hash" }, String(entry.hash || "")),
    entry.subject ? React4.createElement("span", { className: "gg-idletext", title: String(entry.subject) }, String(entry.subject)) : null
  )) : React4.createElement("div", { className: "gg-idletext gg-reference-empty" }, emptyText);
  ...
  return React4.createElement(
    ...
    React4.createElement(
      "div",
      { className: "gg-tab-toolbar" },
      React4.createElement("span", { className: "gg-section-heading" }, "\u5206\u652F\u4E0E\u5F15\u7528"),
      React4.createElement("button", {
        className: "gg-btn",
        disabled: busy || refreshFeedback.state === "loading",
        onClick: () => {
    ...
    message ? React4.createElement("div", { className: "gg-workbench-error" }, message) : null,
    diagnostics ? React4.createElement("pre", { className: "gg-diagnostics" }, diagnostics) : null,
    React4.createElement("div", { className: "gg-reference-tabs", role: "tablist", "aria-label": "Git \u5F15\u7528\u7C7B\u578B" }, [
      { id: "local", label: "\u672C\u5730 (" + branches.length + ")" },
      { id: "remote", label: "\u8FDC\u7A0B (" + remotes.length + ")" },
      { id: "tag", label: "\u6807\u7B7E (" + tags.length + ")" }
    ].map((entry) => React4.createElement("button", {
    ...
    referenceTab === "local" ? React4.createElement(
      "div",
      { className: "gg-local-branches" },
      React4.createElement(
        "label",
        { className: "gg-field", htmlFor: "gg-branch-search" },
        React4.createElement("span", { className: "gg-field-label" }, "\u641C\u7D22\u5206\u652F"),
        React4.createElement("input", {
          id: "gg-branch-search",
          className: "gg-input",
          type: "search",
      ...
      React4.createElement("div", { className: "gg-branch-list" }, visibleBranches.length ? visibleBranches.map((branch, index) => {
        ...
        return React4.createElement(
          ...
          React4.createElement("code", null, (branch.current ? "* " : "") + branchName),
          branch.upstream ? React4.createElement("span", { className: "gg-idletext" }, String(branch.upstream)) : null,
          branch.current ? null : React4.createElement("button", { className: "gg-btn", disabled: busy, onClick: () => {
            cancelDelete();
            mutate("switch-branch", { name: branchName });
          } }, "\u5207\u6362"),
          branch.current || confirming || forcing ? null : React4.createElement("button", {
            className: "gg-btn",
            disabled: busy,
            onClick: () => {
          ...
          confirming ? React4.createElement(
            "div",
            { className: "gg-branch-confirm" },
            React4.createElement("span", null, `\u786E\u5B9A\u5B89\u5168\u5220\u9664\u5206\u652F\u201C${branchName}\u201D\u5417\uFF1F`),
            React4.createElement(
              "div",
              { className: "gg-branch-confirm-actions" },
              React4.createElement("button", { className: "gg-btn danger", disabled: busy, onClick: () => deleteBranch(branchName, false) }, "\u786E\u8BA4\u5B89\u5168\u5220\u9664"),
              React4.createElement("button", { className: "gg-btn", disabled: busy, onClick: cancelDelete }, "\u53D6\u6D88")
            )
          ) : null,
          forcing ? React4.createElement(
            "div",
            { className: "gg-branch-confirm" },
            React4.createElement("div", { className: "gg-riskline" }, `\u5206\u652F\u201C${branchName}\u201D\u5305\u542B\u672A\u5408\u5E76\u63D0\u4EA4\u3002\u5F3A\u5236\u5220\u9664\u53EF\u80FD\u5BFC\u81F4\u8FD9\u4E9B\u63D0\u4EA4\u6C38\u4E45\u4E22\u5931\u3002`),
            React4.createElement(
              "label",
              { className: "gg-check" },
              React4.createElement("input", { type: "checkbox", checked: riskAccepted, disabled: busy, onChange: (event) => setRiskAccepted(!!event.target.checked) }),
              "\u6211\u5DF2\u77E5\u6653\u672A\u5408\u5E76\u63D0\u4EA4\u53EF\u80FD\u6C38\u4E45\u4E22\u5931"
            ),
            React4.createElement(
              "div",
              { className: "gg-branch-confirm-actions" },
              React4.createElement("button", { className: "gg-btn danger", disabled: busy || !riskAccepted, onClick: () => deleteBranch(branchName, true) }, "\u5F3A\u5236\u5220\u9664"),
              React4.createElement("button", { className: "gg-btn", disabled: busy, onClick: cancelDelete }, "\u53D6\u6D88")
            )
          ) : null
        );
      }) : React4.createElement("div", { className: "gg-idletext gg-reference-empty" }, branches.length ? "\u6CA1\u6709\u5339\u914D\u7684\u672C\u5730\u5206\u652F\u3002" : "\u6CA1\u6709\u672C\u5730\u5206\u652F\u3002"))
    ) : referenceTab === "remote" ? React4.createElement("div", { className: "gg-branch-list" }, referenceRows(remotes, "\u6CA1\u6709\u8FDC\u7A0B\u5206\u652F\u3002")) : React4.createElement("div", { className: "gg-branch-list" }, referenceRows(tags, "\u6CA1\u6709\u6807\u7B7E\u3002")),
    referenceTab === "local" ? React4.createElement(
      "div",
      ...
      React4.createElement(
        ...
        React4.createElement("span", { className: "gg-field-label" }, "\u65B0\u5206\u652F\u540D\u79F0"),
        React4.createElement("input", { id: "gg-new-branch-name", className: "gg-input", value: name, placeholder: "\u4F8B\u5982\uFF1Afeature/login", disabled: busy, onChange: (event) => setName(String(event.target.value || "")) })
      ),
      React4.createElement(
        "label",
        { className: "gg-field", htmlFor: "gg-new-branch-base" },
        React4.createElement("span", { className: "gg-field-label" }, "\u57FA\u7840\u5206\u652F"),
        React4.createElement("input", { id: "gg-new-branch-base", className: "gg-input", value: base, placeholder: "\u4F8B\u5982\uFF1Amain", disabled: busy, onChange: (event) => setBase(String(event.target.value || "")) })
      ),
      React4.createElement("button", {
        className: "gg-btn primary",
...
function CommitGraph(props) {
  ...
  const paths = row.edges.map((edge, index) => {
    ...
    if (edge.to === null) {
      return React4.createElement("path", { key: "root-" + index, d: `M ${fromX} 0 L ${fromX} 11`, stroke: color, strokeWidth: 2, fill: "none" });
    }
  ...
  return React4.createElement(
    ...
    paths,
    merge ? React4.createElement("circle", { cx: laneX(row.lane), cy: 11, r: 7, fill: "var(--gg-surface)", stroke: nodeColor, strokeWidth: 2 }) : null,
    React4.createElement("circle", { cx: laneX(row.lane), cy: 11, r: merge ? 3 : 5, fill: nodeColor })
  );
}
function GitCommitsTab(props) {
  const { sessionId, revision } = props;
  const [commits, setCommits] = React4.useState([]);
  const [message, setMessage] = React4.useState("");
  const [selectedHash, setSelectedHash] = React4.useState("");
  const [detail, setDetail] = React4.useState(null);
  const [detailLoading, setDetailLoading] = React4.useState(false);
  const [detailMessage, setDetailMessage] = React4.useState("");
  const [commitDiff, setCommitDiff] = React4.useState(null);
  const [diffLoading, setDiffLoading] = React4.useState(false);
  const [diffMessage, setDiffMessage] = React4.useState("");
  const refreshFeedback = useManualRefreshFeedback();
  const selectedHashRef = React4.useRef("");
  const listRequestRef = React4.useRef({ controller: null, sequence: 0 });
  const detailRequestRef = React4.useRef({ controller: null, sequence: 0 });
  const diffRequestRef = React4.useRef({ controller: null, sequence: 0 });
  const sessionRef = React4.useRef(sessionId);
  ...
  const detailPanel = !selectedHash ? null : React4.createElement(
    "div",
    { className: "gg-commit-detail" },
    React4.createElement(
      "div",
      { className: "gg-commit-detail-head" },
      React4.createElement("strong", { className: "gg-commit-detail-title" }, detail ? String(detail.subject || "\uFF08\u65E0\u63D0\u4EA4\u8BF4\u660E\uFF09") : "\u63D0\u4EA4\u8BE6\u60C5"),
      detail ? React4.createElement("code", { className: "gg-commit-detail-hash", title: String(detail.hash || "") }, String(detail.hash || "").slice(0, 12)) : null,
      React4.createElement("button", {
    ...
    detailLoading ? React4.createElement("div", { className: "gg-idletext" }, "\u6B63\u5728\u52A0\u8F7D\u63D0\u4EA4\u8BE6\u60C5\u2026") : null,
    detailMessage ? React4.createElement("div", { className: "gg-workbench-error" }, detailMessage) : null,
    detail ? React4.createElement(
      React4.Fragment,
      null,
      detail.body ? React4.createElement("pre", { className: "gg-commit-message" }, String(detail.body)) : null,
      React4.createElement(
        "dl",
        { className: "gg-commit-detail-meta" },
        React4.createElement("dt", null, "\u4F5C\u8005"),
        React4.createElement("dd", null, String(detail.authorName || "\u672A\u77E5\u4F5C\u8005") + (detail.authorEmail ? " <" + String(detail.authorEmail) + ">" : "")),
        React4.createElement("dt", null, "\u4F5C\u8005\u65F6\u95F4"),
        React4.createElement("dd", null, String(detail.authoredAt || "\u672A\u77E5")),
        React4.createElement("dt", null, "\u63D0\u4EA4\u8005"),
        React4.createElement("dd", null, String(detail.committerName || "\u672A\u77E5\u63D0\u4EA4\u8005") + (detail.committerEmail ? " <" + String(detail.committerEmail) + ">" : "")),
        React4.createElement("dt", null, "\u63D0\u4EA4\u65F6\u95F4"),
        React4.createElement("dd", null, String(detail.committedAt || "\u672A\u77E5")),
        React4.createElement("dt", null, "\u7236\u63D0\u4EA4"),
        React4.createElement("dd", null, Array.isArray(detail.parents) && detail.parents.length ? detail.parents.map((parent) => parent.slice(0, 12)).join(", ") : "\u6839\u63D0\u4EA4"),
        Array.isArray(detail.parents) && detail.parents.length > 1 ? React4.createElement("dt", null, "\u5DEE\u5F02\u57FA\u7EBF") : null,
        Array.isArray(detail.parents) && detail.parents.length > 1 ? React4.createElement("dd", null, "\u7B2C\u4E00\u7236\u63D0\u4EA4 " + String(detail.comparisonBase || "").slice(0, 12)) : null
      ),
      React4.createElement(
        "div",
        { className: "gg-commit-summary" },
        React4.createElement("span", null, String(detail.totals?.files ?? 0) + " \u4E2A\u6587\u4EF6"),
        React4.createElement("span", { className: "gg-additions" }, "+" + String(detail.totals?.additions ?? 0)),
        React4.createElement("span", { className: "gg-deletions" }, "-" + String(detail.totals?.deletions ?? 0)),
        detail.totals?.binary ? React4.createElement("span", null, String(detail.totals.binary) + " \u4E2A\u4E8C\u8FDB\u5236\u6587\u4EF6") : null
      ),
      React4.createElement("div", { className: "gg-commit-files" }, Array.isArray(detail.files) && detail.files.length ? detail.files.map((file, index) => React4.createElement(
        "div",
        {
          className: "gg-commit-file" + commitFileTone(String(file.status || "")),
          key: String(file.path || index)
        },
        React4.createElement("code", null, String(file.status || "?")),
        React4.createElement(
          "span",
        ...
        React4.createElement("span", { className: "gg-additions" }, file.additions === null ? "\u4E8C\u8FDB\u5236" : "+" + String(file.additions)),
        React4.createElement("span", { className: "gg-deletions" }, file.deletions === null ? "" : "-" + String(file.deletions))
      )) : React4.createElement("div", { className: "gg-idletext gg-reference-empty" }, "\u8BE5\u63D0\u4EA4\u6CA1\u6709\u53EF\u663E\u793A\u7684\u6587\u4EF6\u53D8\u66F4\u3002")),
      detail.filesTruncated ? React4.createElement("div", { className: "gg-riskline" }, "\u6587\u4EF6\u5217\u8868\u8FC7\u957F\uFF0C\u4EC5\u663E\u793A\u524D 500 \u9879\u3002") : null,
      React4.createElement(
        "button",
      ...
      diffMessage ? React4.createElement("div", { className: "gg-workbench-error" }, diffMessage) : null,
      commitDiff ? React4.createElement(
        React4.Fragment,
        null,
        commitDiff.truncated ? React4.createElement("div", { className: "gg-riskline" }, "Diff \u8FC7\u957F\uFF0C\u5DF2\u622A\u65AD\u4E3A\u524D 300,000 \u4E2A\u5B57\u7B26\u3002") : null,
        React4.createElement(
          "pre",
  ...
  return React4.createElement(
    ...
    React4.createElement(
      "div",
      { className: "gg-tab-toolbar" },
      React4.createElement("span", { className: "gg-section-heading" }, "\u63D0\u4EA4\u8BB0\u5F55", React4.createElement("span", { className: "gg-section-count" }, String(commits.length))),
      React4.createElement("button", {
        className: "gg-btn",
    ...
    React4.createElement(
      ...
      React4.createElement("div", { className: "gg-commit-list" }, rows.length ? rows.map((row, index) => {
        ...
        return React4.createElement(
          ...
          React4.createElement(CommitGraph, { row }),
          React4.createElement(
            "div",
            ...
            React4.createElement(
              ...
              React4.createElement("span", { className: "gg-commit-subject" }, String(commit.subject || "\uFF08\u65E0\u63D0\u4EA4\u8BF4\u660E\uFF09")),
              React4.createElement("span", { className: "gg-commit-refs" }, refs.map((ref, refIndex) => React4.createElement("span", {
            ...
            React4.createElement(
              ...
              React4.createElement("span", { className: "gg-commit-author" }, String(commit.author || "\u672A\u77E5\u4F5C\u8005")),
              React4.createElement("code", { className: "gg-commit-hash" }, hash.slice(0, 8))
            )
          )
        );
      }) : React4.createElement("div", { className: "gg-idletext" }, "\u6CA1\u6709\u63D0\u4EA4\u8BB0\u5F55\u3002")),
      detailPanel
    )
  );
}
function GitSyncTab(props) {
  ...
  const [targets, setTargets] = React4.useState([]);
  const [remote, setRemote] = React4.useState("");
  const [rebaseTarget, setRebaseTarget] = React4.useState("");
  const [riskAccepted, setRiskAccepted] = React4.useState(false);
  const [busy, setBusy] = React4.useState(false);
  const [message, setMessage] = React4.useState("");
  const [diagnostics, setDiagnostics] = React4.useState("");
  ...
  return React4.createElement(
    ...
    React4.createElement(
      "div",
      { className: "gg-tab-toolbar" },
      state ? renderRepositoryIdentity(state.topLevel, state.branch) : React4.createElement("span", { className: "gg-section-heading" }, "\u6B63\u5728\u8BFB\u53D6\u4ED3\u5E93\u2026"),
      React4.createElement("button", {
        className: "gg-btn",
        type: "button",
        disabled: busy || refreshFeedback.state === "loading",
    ...
    message ? React4.createElement("div", { className: "gg-workbench-error" }, message) : null,
    diagnostics ? React4.createElement("pre", { className: "gg-diagnostics" }, diagnostics) : null,
    state ? React4.createElement(
      "div",
      { className: "gg-sync-grid" },
      React4.createElement("div", { className: "gg-sync-card" }, React4.createElement("strong", null, "\u4E0A\u6E38\u5206\u652F"), React4.createElement("code", { className: "gg-sync-value" }, state.upstream || "\u672A\u8BBE\u7F6E")),
      React4.createElement("div", { className: "gg-sync-card" }, React4.createElement("strong", null, "\u63D0\u4EA4\u5DEE\u5F02"), React4.createElement("span", { className: "gg-sync-value" }, "\u9886\u5148 " + state.ahead + " \xB7 \u843D\u540E " + state.behind)),
      React4.createElement("div", { className: "gg-sync-card" }, React4.createElement("strong", null, "\u5DE5\u4F5C\u533A"), React4.createElement("span", { className: "gg-sync-value" }, statusText)),
      React4.createElement("div", { className: "gg-sync-card" }, React4.createElement("strong", null, "\u8FDC\u7A0B\u4ED3\u5E93"), React4.createElement("span", { className: "gg-sync-value" }, state.remotes.join("\u3001") || "\u672A\u914D\u7F6E"))
    ) : null,
    React4.createElement("button", { className: "gg-btn", type: "button", onClick: props.onConflicts }, "\u89E3\u51B3\u51B2\u7A81 / Merge / Cherry-pick"),
    !hasRemote && state ? React4.createElement("div", { className: "gg-sync-warning" }, "\u5F53\u524D\u4ED3\u5E93\u6CA1\u6709\u8FDC\u7A0B\u4ED3\u5E93\u3002\u8BF7\u5148\u5728\u7EC8\u7AEF\u6216\u540E\u7EED\u7684 Remote \u7BA1\u7406\u529F\u80FD\u4E2D\u6DFB\u52A0\u8FDC\u7A0B\u5730\u5740\u3002") : null,
    React4.createElement(
      "div",
      { className: "gg-sync-actions" },
      React4.createElement("strong", null, "\u8FDC\u7A0B\u540C\u6B65"),
      React4.createElement(
        "div",
        ...
        React4.createElement(
          ...
          React4.createElement("span", { className: "gg-field-label" }, "\u8FDC\u7A0B\u4ED3\u5E93"),
          ...
          }, state ? state.remotes.map((entry) => React4.createElement("option", { value: entry, key: entry }, entry)) : null)
        ),
        React4.createElement("button", { className: "gg-btn", type: "button", disabled: busy || !remote, onClick: () => {
      ...
      React4.createElement("div", { className: "gg-sync-note" }, "Pull \u56FA\u5B9A\u6267\u884C git pull --ff-only\uFF1B\u82E5\u4E0D\u80FD\u5FEB\u8FDB\u4F1A\u505C\u6B62\u5E76\u4FDD\u7559\u5B8C\u6574\u9519\u8BEF\uFF0C\u4E0D\u4F1A\u81EA\u52A8\u5408\u5E76\u3002")
    ),
    React4.createElement(
      "div",
      { className: "gg-sync-actions" },
      React4.createElement("strong", null, "Rebase\uFF08\u9AD8\u98CE\u9669\uFF09"),
      state?.rebaseInProgress ? React4.createElement("div", { className: "gg-sync-warning" }, state.conflictCount > 0 ? "\u8BF7\u5230\u201C\u51B2\u7A81\u89E3\u51B3\u201D\u9875\u5904\u7406\u5E76\u6807\u8BB0\u5168\u90E8\u51B2\u7A81\uFF0C\u7136\u540E\u7EE7\u7EED Rebase\u3002" : "\u51B2\u7A81\u5DF2\u7ECF\u89E3\u51B3\u5E76\u6682\u5B58\uFF0C\u53EF\u4EE5\u7EE7\u7EED Rebase\uFF1B\u4E5F\u53EF\u4EE5\u4E2D\u6B62\u5E76\u6062\u590D\u5230\u5F00\u59CB\u524D\u3002") : null,
      !state?.rebaseInProgress ? React4.createElement(
        "label",
        { className: "gg-field", htmlFor: "gg-rebase-target" },
        React4.createElement("span", { className: "gg-field-label" }, "\u76EE\u6807\u5F15\u7528"),
        React4.createElement("select", {
          id: "gg-rebase-target",
          className: "gg-input",
          value: rebaseTarget,
        ...
        }, targets.map((entry) => React4.createElement("option", { value: entry, key: entry }, entry)))
      ) : null,
      React4.createElement(
        "label",
        { className: "gg-check" },
        ...
        React4.createElement("span", null, state?.rebaseInProgress ? "\u6211\u4E86\u89E3\u7EE7\u7EED\u6216\u4E2D\u6B62 Rebase \u53EF\u80FD\u6539\u5199\u5386\u53F2\u6216\u4E22\u5F03\u672C\u6B21\u51B2\u7A81\u5904\u7406" : "\u6211\u4E86\u89E3 Rebase \u4F1A\u91CD\u5199\u5F53\u524D\u5206\u652F\u7684\u672C\u5730\u63D0\u4EA4\u5386\u53F2")
      ),
      state?.rebaseInProgress ? React4.createElement(
        "div",
        { className: "gg-actions" },
      ...
      React4.createElement("div", { className: "gg-sync-note" }, state?.dirty && !state.rebaseInProgress ? "\u5DE5\u4F5C\u533A\u6709\u6539\u52A8\uFF1A\u4E3A\u907F\u514D\u4E22\u5931\u5185\u5BB9\uFF0C\u5F00\u59CB Rebase \u5DF2\u7981\u7528\u3002" : "Rebase \u53D1\u751F\u51B2\u7A81\u65F6\u4F1A\u4FDD\u7559\u5728\u5F53\u524D\u9875\u9762\uFF1B\u9519\u8BEF\u8BE6\u60C5\u4F1A\u4EA4\u7ED9\u73B0\u6709\u5EFA\u8BAE\u4E0E Agent \u5206\u6790\u6D41\u7A0B\u3002")
    )
  );
}
function GitWorkbenchPanel(props) {
  const [tab, setTab] = React4.useState("changes");
  const [conflictDirty, setConflictDirty] = React4.useState(false);
  const conflictDirtyRef = React4.useRef(false);
  conflictDirtyRef.current = conflictDirty;
  const [revision, setRevision] = React4.useState(0);
  const [commandLogs, setCommandLogs] = React4.useState([]);
  const [pendingAnalysis, setPendingAnalysis] = React4.useState(null);
  const commandSeqRef = React4.useRef(0);
  const commandLogBodyRef = React4.useRef(null);
  const delayedOpenDisposers = React4.useRef([]);
  const observedProposalIdRef = React4.useRef(null);
  const proposalStateRequestRef = React4.useRef({ controller: null, sequence: 0 });
  ...
  const content = tab === "changes" ? React4.createElement(GitChangesTab, { sessionId: props.sessionId, intervalFn: props.intervalFn, revision, onChanged: refresh, onCommand: reportCommand, onFailure: handleFailure, onConflicts: () => setTab("conflicts") }) : tab === "conflicts" ? React4.createElement(GitConflictsTab, { key: props.sessionId, sessionId: props.sessionId, revision, rpc, onChanged: refresh, onDirty: setConflictDirty, onCommand: reportCommand }) : tab === "branches" ? React4.createElement(GitBranchesTab, { sessionId: props.sessionId, revision, onChanged: refresh, onCommand: reportCommand, onFailure: handleFailure }) : tab === "merge" ? React4.createElement(GitMergeTab, { key: props.sessionId, sessionId: props.sessionId, revision, rpc, onChanged: refresh, onCommand: reportCommand, onConflicts: () => setTab("conflicts"), renderReview: renderReviewSurface, renderRawDiff: renderRawDiffSurface }) : tab === "commits" ? React4.createElement(GitCommitsTab, { sessionId: props.sessionId, revision }) : tab === "stashes" ? React4.createElement(GitStashesTab, { key: props.sessionId, sessionId: props.sessionId, revision, rpc, onChanged: refresh, onCommand: reportCommand, onConflicts: () => setTab("conflicts"), renderReview: renderReviewSurface, renderRawDiff: renderRawDiffSurface }) : tab === "sync" ? React4.createElement(GitSyncTab, { sessionId: props.sessionId, intervalFn: props.intervalFn, revision, onChanged: refresh, onCommand: reportCommand, onFailure: handleFailure, onConflicts: () => setTab("conflicts") }) : React4.createElement(GitDock, { sessionId: props.sessionId, intervalFn: props.intervalFn, timeoutFn: props.timeoutFn, onFailure: handleFailure });
  const analysisBanner = shouldShowAnalysisBanner(tab, pendingAnalysis) && pendingAnalysis ? React4.createElement(
    "div",
    { className: "gg-analysis" },
    React4.createElement("strong", null, pendingAnalysis.status === "waiting" ? "Agent \u6B63\u5728\u5206\u6790 Git \u5931\u8D25\u2026" : "\u8FD9\u4E2A Git \u5931\u8D25\u9700\u8981 Agent \u5206\u6790"),
    renderFailureDetails(pendingAnalysis.failure),
    pendingAnalysis.error ? React4.createElement("div", { className: "gg-workbench-error" }, pendingAnalysis.error) : null,
    React4.createElement("div", { className: "gg-idletext" }, pendingAnalysis.status === "waiting" ? "\u5DF2\u53D1\u9001\u7ED9\u5F53\u524D\u4F1A\u8BDD Agent\uFF1B\u5B83\u751F\u6210\u53EF\u6267\u884C\u63D0\u8BAE\u540E\u4F1A\u81EA\u52A8\u8DF3\u8F6C\u5230\u5EFA\u8BAE\u9875\u3002" : "\u786E\u8BA4\u540E\uFF0CAgent \u4F1A\u8BFB\u53D6\u4ED3\u5E93\u3001\u6587\u4EF6\u548C\u8FDC\u7A0B\u8DDF\u8E2A\u72B6\u6001\uFF0C\u4EC5\u751F\u6210\u4FEE\u590D\u63D0\u8BAE\uFF0C\u4E0D\u4F1A\u76F4\u63A5\u6267\u884C\u3002"),
    pendingAnalysis.status !== "waiting" ? React4.createElement(
      "div",
      { className: "gg-actions" },
      React4.createElement("button", {
        className: "gg-btn primary",
  ...
  return React4.createElement(
    ...
    React4.createElement(
      ...
      React4.createElement("span", { className: "gg-workbench-title" }, "Git \u5DE5\u4F5C\u53F0"),
      React4.createElement("button", {
        type: "button",
    ...
    React4.createElement(
      ...
      React4.createElement("div", { className: "gg-tabs", role: "tablist", "aria-label": "Git \u5DE5\u4F5C\u53F0\u533A\u57DF" }, tabs.map((entry, index) => React4.createElement("button", {
    ...
    React4.createElement(
      ...
      React4.createElement("strong", { className: "gg-command-log-head" }, "\u547D\u4EE4\u65E5\u5FD7"),
      React4.createElement("div", { className: "gg-command-log-body", ref: commandLogBodyRef }, commandLogs.length ? commandLogs.map((entry) => React4.createElement(
        "div",
        { className: "gg-command-entry", key: entry.id },
        React4.createElement("span", { className: "gg-command-label" }, entry.label),
        React4.createElement("span", { className: "gg-command-status " + entry.status }, entry.status === "running" ? "\u6267\u884C\u4E2D" : entry.status === "succeeded" ? "\u6210\u529F" : "\u5931\u8D25"),
        React4.createElement("code", { className: "gg-command-code" }, entry.command)
      )) : React4.createElement("div", { className: "gg-idletext" }, "\u5C1A\u672A\u6267\u884C\u4FEE\u6539\u547D\u4EE4\u3002"))
    )
  );
}
function GitDock(props) {
  ...
  const [view, setView] = React4.useState(null);
  const [busy, setBusy] = React4.useState(false);
  const [understood, setUnderstood] = React4.useState(false);
  const [outcome, setOutcome] = React4.useState(null);
  const [ranInfo, setRanInfo] = React4.useState(null);
  const [partialInfo, setPartialInfo] = React4.useState(null);
  const [verifyMsg, setVerifyMsg] = React4.useState(null);
  const [collapsed, setCollapsed] = React4.useState(false);
  const currentProposalId = React4.useRef(null);
  const scheduledCloses = React4.useRef(/* @__PURE__ */ new Set());
  const closeDisposers = React4.useRef([]);
  const recoveryRefreshDisposers = React4.useRef([]);
  const deferredRecoveryUntil = React4.useRef(0);
  ...
  if (!view) {
    return React4.createElement(
      ...
      React4.createElement(
        "div",
        { className: "gg-idle" },
        React4.createElement("span", { className: "gg-badge normal" }, "Git \u64CD\u4F5C\u5EFA\u8BAE \xB7 \u7A7A\u95F2"),
        React4.createElement("span", { className: "gg-idletext" }, "\u7B49\u5F85\u65B0\u7684 git \u64CD\u4F5C\u63D0\u8BAE\u2026\uFF08git_propose \u767B\u8BB0\u540E\u8FD9\u91CC\u4F1A\u51FA\u73B0\u547D\u4EE4\u4E0E\u6309\u94AE\uFF09")
      )
    );
  }
  const proposal = view;
  ...
  const renderSteps = () => steps.map((s, i) => React4.createElement(
    "div",
    { className: "gg-step", key: "step" + i },
    React4.createElement("span", { className: "gg-stepnum" }, String(i + 1) + "."),
    React4.createElement("code", { className: "gg-stepcode" }, String(s.command))
  ));
  const headerEl = React4.createElement(
    "div",
    { className: "gg-head", key: "head" },
    React4.createElement("span", null, "Git \u64CD\u4F5C\u5EFA\u8BAE"),
    React4.createElement("span", { className: badgeCls }, riskLabel),
    React4.createElement(
      "button",
  ...
  if (collapsed) {
    return React4.createElement("div", { className: "gg-dock gg-dock-full" }, headerEl);
  }
  const lines = [headerEl];
  if (proposal.intent) lines.push(React4.createElement("div", { className: "gg-intent", key: "intent" }, String(proposal.intent)));
  lines.push(React4.createElement("div", { className: "gg-steps", key: "steps" }, renderSteps()));
  if (proposal.failure) {
    lines.push(React4.createElement("div", { key: "failure" }, renderFailureDetails(proposal.failure, proposal.recoverySuggestion || "")));
  } else if (proposal.explanation) {
    lines.push(React4.createElement("div", { className: "gg-expl", key: "expl" }, String(proposal.explanation)));
  }
  if (ranInfo) {
    lines.push(React4.createElement("div", { className: "gg-ok gg-ran", key: "ran" }, "\u2714 \u68C0\u6D4B\u5230\u9884\u671F\u7ED3\u679C\u5DF2\u8FBE\u6210\uFF08\u5DF2\u6267\u884C\uFF09\uFF0C\u5373\u5C06\u5173\u95ED\u6B64\u5EFA\u8BAE"));
    lines.push(React4.createElement("pre", { className: "gg-pre", key: "ranstate" }, String(ranInfo)));
    return React4.createElement("div", { className: "gg-dock gg-dock-full" }, lines);
  }
  if (partialInfo) {
    lines.push(React4.createElement("div", { className: "gg-riskline", key: "pmsg" }, "\u26A0 " + String(partialInfo.message)));
    if (partialInfo.state) lines.push(React4.createElement("pre", { className: "gg-pre", key: "pstate" }, String(partialInfo.state)));
    const acts = [];
    acts.push(React4.createElement("button", { key: "verify", className: "gg-btn primary", onClick: onVerify }, "\u91CD\u65B0\u68C0\u6D4B"));
    acts.push(React4.createElement("button", { key: "drop", className: "gg-btn", onClick: () => doDismiss(proposal.proposalId) }, "\u653E\u5F03\u5EFA\u8BAE"));
    lines.push(React4.createElement("div", { className: "gg-actions", key: "actions" }, acts));
    return React4.createElement("div", { className: "gg-dock gg-dock-full" }, lines);
  }
  if (isCopied && isPending) {
    lines.push(React4.createElement("div", { className: "gg-intent", key: "copied" }, "\u547D\u4EE4\u5DF2\u7528 && \u8FDE\u63A5\u540E\u590D\u5236\uFF0C\u4EFB\u4E00\u6B65\u5931\u8D25\u90FD\u4F1A\u505C\u6B62\u3002\u9762\u677F\u4F1A\u6BD4\u5BF9\u590D\u5236\u524D\u540E\u7684\u76EE\u6807\u72B6\u6001\u3002"));
    if (verifyMsg) lines.push(React4.createElement("div", { className: "gg-riskline", key: "vmsg" }, verifyMsg));
    const acts = [];
    acts.push(React4.createElement("button", { key: "verify", className: "gg-btn primary", onClick: onVerify }, "\u91CD\u65B0\u68C0\u6D4B"));
    acts.push(React4.createElement("button", { key: "drop", className: "gg-btn", onClick: () => doDismiss(proposal.proposalId) }, "\u653E\u5F03\u5EFA\u8BAE"));
    lines.push(React4.createElement("div", { className: "gg-actions", key: "actions" }, acts));
  ...
  if (proposal.status === "running") {
    lines.push(React4.createElement("div", { className: "gg-intent", key: "running" }, "\u547D\u4EE4\u6B63\u5728\u6267\u884C\uFF0C\u8BF7\u52FF\u91CD\u590D\u63D0\u4EA4\u2026"));
  } else if (proposal.status === "failed") {
    lines.push(React4.createElement("div", { className: "gg-riskline", key: "failed" }, "\u8BE5\u63D0\u8BAE\u5DF2\u7ECF\u5931\u8D25\u5E76\u9501\u5B9A\u3002\u8BF7\u6839\u636E\u8BCA\u65AD\u521B\u5EFA\u4FEE\u6B63\u63D0\u8BAE\uFF0C\u4E0D\u4F1A\u81EA\u52A8\u91CD\u653E\u3002"));
    if (canDismissFailedProposal(proposal.needsAgentAnalysis)) {
      lines.push(React4.createElement(
        "div",
        { className: "gg-actions", key: "failed-actions" },
        React4.createElement("button", { className: "gg-btn", onClick: () => doDismiss(proposal.proposalId) }, "\u5173\u95ED\u5931\u8D25\u63D0\u8BAE")
      ));
    }
  } else if (isPending) {
    if (isHard) {
      lines.push(React4.createElement("div", { className: "gg-riskline", key: "risk" }, "\u26A0 " + (proposal.reasons && proposal.reasons.length ? proposal.reasons.join("\uFF1B") : "\u8BE5\u64CD\u4F5C\u98CE\u9669\u8F83\u9AD8\uFF0C\u53EF\u80FD\u9020\u6210\u4E0D\u53EF\u9006\u7684\u6539\u52A8")));
      lines.push(React4.createElement(
        "label",
        { className: "gg-check", key: "ck" },
        React4.createElement("input", { type: "checkbox", checked: understood, onChange: (e) => setUnderstood(e.target.checked) }),
        React4.createElement("span", null, "\u6211\u5DF2\u4E86\u89E3\u98CE\u9669\uFF0C\u786E\u8BA4\u6267\u884C\u6216\u590D\u5236")
      ));
    }
    const actions = [];
    actions.push(React4.createElement(
      "button",
      { key: "run", className: "gg-btn " + (isHard ? "danger" : "primary"), disabled: !canRun, onClick: onRun },
      busy ? "\u6267\u884C\u4E2D\u2026" : isHard ? "\u786E\u8BA4\u5E76\u76F4\u63A5\u6267\u884C" : "\u76F4\u63A5\u6267\u884C"
    ));
    actions.push(React4.createElement("button", { key: "copy", className: "gg-btn", disabled: !canCopy, onClick: onCopy }, "\u590D\u5236\u547D\u4EE4\uFF08\u624B\u52A8\u6267\u884C\uFF09"));
    lines.push(React4.createElement("div", { className: "gg-actions", key: "actions" }, actions));
  }
  if (outcome) {
    ...
    lines.push(React4.createElement(
      ...
      React4.createElement("div", { className: ok ? "gg-ok" : "gg-fail" }, ok ? "\u2714 \u6267\u884C\u6210\u529F" : "\u2718 \u6267\u884C\u5931\u8D25"),
      React4.createElement("pre", null, text)
    ));
    if (outcome.steps && outcome.steps.length) {
      const stepLines = outcome.steps.map((s, i) => React4.createElement(
        "div",
        { className: "gg-stepres", key: "sr" + i },
        React4.createElement("span", { className: s.ok ? "gg-ok" : "gg-fail" }, s.ok ? "\u2713" : "\u2717"),
        React4.createElement("code", { className: "gg-stepcode" }, String(s.command))
      ));
      lines.push(React4.createElement("div", { className: "gg-out", key: "stepsout" }, stepLines));
    }
    if (!ok) {
      if (outcome.diagnostics) lines.push(React4.createElement(
        "div",
        { className: "gg-out", key: "diag" },
        React4.createElement("div", { className: "gg-intent" }, "\u4ED3\u5E93\u8BCA\u65AD\u4FE1\u606F\uFF1A"),
        React4.createElement("pre", null, String(outcome.diagnostics))
      ));
      if (outcome.recovery && outcome.recovery.command) {
        lines.push(React4.createElement(
          "div",
          { className: "gg-out gg-recovery", key: "recovery" },
          React4.createElement("div", { className: "gg-ok" }, "\u{1F4A1} \u5DF2\u751F\u6210\u4FEE\u6B63\u5EFA\u8BAE" + (outcome.recovery.proposalId ? "\uFF08\u5DF2\u767B\u8BB0\u4E3A\u65B0\u7684\u63D0\u8BAE\uFF0C\u53EF\u76F4\u63A5\u6267\u884C\uFF09" : "")),
          React4.createElement("div", { className: "gg-intent" }, String(outcome.recovery.suggestion || "")),
          React4.createElement("code", { className: "gg-stepcode" }, String(outcome.recovery.command))
        ));
      } else if (outcome.recovery && outcome.recovery.suggestion) {
        lines.push(React4.createElement(
          "div",
          { className: "gg-out gg-recovery", key: "recovery" },
          React4.createElement("div", { className: "gg-riskline" }, "\u{1F4A1} " + String(outcome.recovery.suggestion))
        ));
      } else {
        lines.push(React4.createElement("div", { className: "gg-intent", key: "hint" }, "\u6267\u884C\u5931\u8D25\u3002\u4F60\u53EF\u4EE5\u63CF\u8FF0\u4E0B\u4E00\u6B65\uFF0C\u6216\u8BA9\u6211\u5206\u6790\u539F\u56E0\u5E76\u7ED9\u51FA\u4FEE\u6B63\u547D\u4EE4\u3002"));
      }
    }
  }
  return React4.createElement("div", { className: "gg-dock gg-dock-full" }, lines);
...
var plugin = {
  ...
  apply(ctx) {
    ...
    const openWorkbench = registerWorkbench(ctx, (panelProps) => React4.createElement(GitWorkbenchPanel, {
      ...panelProps,
      intervalFn,
      timeoutFn
    }));
    slots.inject("conversation.input.left", () => slots.register(
      ...
      (props) => React4.createElement(GitWorkbenchAction, { sessionId: props.sessionId, openWorkbench, intervalFn })
    ));
  },
  __testing: {
    GitMergeTab,
