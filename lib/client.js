...
function GitWorkbenchPanel(props) {
  ...
  const requestAnalysis = () => {
    ...
    }).then(() => setPendingAnalysis((active) => active && active.proposalId === current.proposalId ? { ...active, status: "waiting", error: "" } : active)).catch((error) => setPendingAnalysis((active) => active && active.proposalId === current.proposalId ? { ...active, status: "ready", error: errorText(error) } : active));
  };
  const abandonAnalysis = () => {
    ...
    }).catch((error) => setPendingAnalysis((active) => active && active.proposalId === current.proposalId ? { ...active, status: "ready", error: errorText(error) } : active));
  };
...
function GitDock(props) {
  ...
  const refresh = () => {
    rpc({ action: "state", sessionId: String(sessionId) }).then((res) => {
      ...
      console.log("easygit state \u8C03\u7528\u5931\u8D25", errorText(err));
    });
  };
  React.useEffect(() => {
  ...
  const onRun = () => {
    ...
    rpc({ action: "execute", sessionId: String(sessionId), proposalId: proposal.proposalId, confirm: understood }).then((res) => {
      ...
      setOutcome({ ok: false, error: errorText(err) });
    }).then(() => setBusy(false));
  };
  const onCopy = () => {
    try {
...
var plugin = {
  ...
  apply(ctx) {
    ...
    slots.inject("conversation.input.left", () => slots.register(
      ...
      (props) => React.createElement(GitWorkbenchAction, { sessionId: props.sessionId, openWorkbench, intervalFn })
    ));
  },
  __testing: {
    registerWorkbench,
