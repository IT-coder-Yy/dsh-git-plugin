...
在 **变更 → 提交撤销与修正** 中操作最近提交；也可在 **提交记录 → 点击提交** 后操作，历史提交提供 Revert。

- **修改最近提交说明**：预填完整说明，支持多行（最多 48 KiB）；仅修改说明，保留暂存区和工作区内容。
- **补充最近提交**：先暂存要补充的文件，再将全部已暂存改动纳入最近提交，保留原说明；未暂存改动不会提交。
- **撤销最近提交但保留修改**：使用 `reset --soft` 退回第一父提交，撤销的改动保留在暂存区，已有暂存内容和工作区文件保持不变。根提交不支持此操作。
- **Revert 此提交**：为当前分支历史中的指定提交创建反向提交，要求工作区和暂存区干净。合并提交必须选择主线父提交，撤销相对该父提交的改动。操作因冲突暂停后进入“冲突解决”，可以继续、中止或跳过；没有可撤销改动时展示 Git 诊断。

操作前需确认影响。前三项会改写本地历史；已推送提交的修正会影响远程同步，不会自动强制推送。分支、HEAD 或暂存区在读取后变化时会拒绝执行，需刷新后重新确认；进行中的 Git 操作或未解决冲突也会阻止修正。执行后自动刷新相关视图。
...
1. 查看冲突文件列表和当前 Merge / Rebase / Cherry-pick / Revert 状态；也可以输入分支或提交引用发起 Merge、Rebase 或 Cherry-pick。Revert 从提交详情发起。
