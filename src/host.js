/**
 * deepseek-git-guide — Host half（Host 半区）
 *
 * 动态 Cordis 插件，运行在 DeepSeek Harness 的 Host 进程。
 * 本文件内容即 cordis_define 的 code.host 函数体（`module.exports` 即返回的插件对象），
 * 加载方式见 README「如何安装」。
 *
 * 提供的模型工具：
 ...
 *
 * 提供的 Client RPC（浏览器面板调用）：
 *   - git:panel-state / git:execute / git:dismiss / git:mark-copied / git:verify
 */
...
module.exports = {
  ...
  apply(ctx) {
    ...
    harness.registerTool(ctx, harness.defineTool({
      ...
      parameters: {
        intent: { type: 'string', required: true, description: '用户想要完成的 git 操作意图（自然语言，简短描述）' },
        ...
        explanation: { type: 'string', required: true, description: '为什么用这些命令：它们做什么、为什么最简洁安全、有什么副作用' },
    ...
    harness.registerTool(ctx, harness.defineTool({
      ...
      parameters: {
        proposalId: { type: 'string', required: true, description: 'git_propose 返回的 proposalId' },
    ...
    harness.handle('git:panel-state', async (args) => {
      ...
      if (p && p.copied === true && p.fingerprint) {
        ...
        return { ok: true, proposal: view, changed: v.changed, verified: v.verified, partial: v.partial, message: v.message, changedState: v.changedState }
      }
      return { ok: true, proposal: view, changed: false, verified: false, partial: false, message: '', changedState: '' }
    })
    harness.handle('git:dismiss', async (args) => {
      const sessionId = args && typeof args.sessionId === 'string' ? args.sessionId : 'unknown'
      const proposal = findProposal(sessionId, args && args.proposalId, true)
      if (!proposal) return { ok: false, error: '找不到该提议' }
      proposal.closed = true
      proposal.manual = args && args.manual === true
      return { ok: true }
    })
    harness.handle('git:mark-copied', async (args) => {
      const sessionId = args && typeof args.sessionId === 'string' ? args.sessionId : 'unknown'
      const proposal = findProposal(sessionId, args && args.proposalId, true)
      if (!proposal) return { ok: false, error: '找不到该提议' }
    ...
    harness.handle('git:verify', async (args) => {
      const sessionId = args && typeof args.sessionId === 'string' ? args.sessionId : 'unknown'
      const proposal = findProposal(sessionId, args && args.proposalId, true)
      if (!proposal) return { ok: false, error: '找不到该提议' }
      if (proposal.copied !== true || !proposal.fingerprint) return { ok: true, changed: false, verified: false, partial: false, message: '尚未复制命令或缺少对比基线', changedState: '' }
      const v = await verifyProposal(proposal)
      if (v.verified) proposal.verified = true
      return { ok: true, changed: v.changed, verified: v.verified, partial: v.partial, message: v.message, changedState: v.changedState }
    })
    harness.handle('git:execute', async (args) => {
      const sessionId = args && typeof args.sessionId === 'string' ? args.sessionId : 'unknown'
      const proposal = findProposal(sessionId, args && args.proposalId, true)
      if (!proposal) return { ok: false, proposalId: args && args.proposalId ? String(args.proposalId) : '', command: '', steps: [], exitCode: -1, signal: '', timedOut: false, stdout: '', stderr: '', diagnostics: '', error: '找不到该提议（proposalId 无效或已过期）' }
      if (proposal.risk === 'hard' && !(args && args.confirm === true)) {
        return { ok: false, proposalId: proposal.proposalId, command: proposal.command, steps: [], exitCode: -1, signal: '', timedOut: false, stdout: '', stderr: '', diagnostics: '', error: '高风险操作：请先勾选“我已了解风险”再执行' }
      }
      if (!shell) return { ok: false, proposalId: proposal.proposalId, command: proposal.command, steps: [], exitCode: -1, signal: '', timedOut: false, stdout: '', stderr: '', diagnostics: '', error: 'shell 服务不可用' }
      const { ok, stepsResult } = await executeProposalSteps(shell, proposal)
      const last = proposal.steps[proposal.steps.length - 1]
      ...
      const lastResult = failedStep ? failedStep.result : (last ? last.result : null)
      const diagnostics = ok ? '' : await captureDiagnostics(shell, proposal.workdir)
      console.log('git:execute RPC 执行', proposal.proposalId, 'ok=', ok, 'steps=', stepsResult.length)
      return {
        ok,
        proposalId: proposal.proposalId,
        command: proposal.command,
