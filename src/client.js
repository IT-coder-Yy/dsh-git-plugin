/**
 * deepseek-git-guide — Client half（Client 半区 / 浏览器端）
 *
 * 动态 Cordis 插件，运行在 DeepSeek Harness 的浏览器页面。
 * 本文件内容即 cordis_define 的 code.client 函数体（`module.exports` 即返回的插件对象）。
 *
 * 运行时由动态 Client Runner 提供的全局：React、host（Package 私有 RPC）、styles。
 * 面板注册在 conversation.input.dock（输入框上方通栏），始终可见：
...
function GitDock(props) {
  ...
  const refresh = () => {
    host.call('git:panel-state', { sessionId: String(sessionId) })
      .then((res) => {
        if (res && res.ok === true) {
      ...
      .catch((err) => { console.log('git:panel-state 调用失败', String((err && err.message) || err)) })
  }
  ...
  const doDismiss = (pid) => {
    host.call('git:dismiss', { sessionId: String(sessionId), proposalId: pid })
      .then(refresh)
      .catch(refresh)
  }
  ...
  const onRun = () => {
    ...
    setBusy(true)
    setOutcome(null)
    host.call('git:execute', { sessionId: String(sessionId), proposalId: proposal.proposalId, confirm: understood })
      .then((res) => {
    ...
    try {
      ...
      if (nav && nav.clipboard && typeof nav.clipboard.writeText === 'function') {
        ...
        nav.clipboard.writeText(text)
          .then(() => host.call('git:mark-copied', { sessionId: String(sessionId), proposalId: proposal.proposalId }).then(refresh).catch(refresh))
          .catch(() => {})
      }
    } catch (e) { /* ignore */ }
  }
  const onVerify = () => {
    setVerifyMsg(null)
    setPartialInfo(null)
    host.call('git:verify', { sessionId: String(sessionId), proposalId: proposal.proposalId })
      .then((res) => {
